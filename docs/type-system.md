...
Over `graphql-transport-ws`, a `@defer` or `@stream` field is delivered using the `pending`/`incremental`/`completed`/`hasNext` format used by graphql-js 17 and Apollo Client's `GraphQL17Alpha9Handler`. Each field is announced once, in a `pending` entry, the first time it produces a result, and identified afterwards by a short id rather than its path. A streamed field's items always arrive in list order, and a batch of items is delivered as the `items` of one `incremental` entry addressed by that id.
