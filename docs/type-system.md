...
2. `coerceValue` function, which will return `Some` only if it fits into scalar's allowed values range, otherwise `None`. Applied while producing return object graph from resolver's outout.
