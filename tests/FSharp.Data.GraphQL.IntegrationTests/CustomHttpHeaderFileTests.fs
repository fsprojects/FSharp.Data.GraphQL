module FSharp.Data.GraphQL.IntegrationTests.CustomHttpHeaderFileTests
...
// We should be able to provide custom HTTP headers through a header file.
type Provider = GraphQLProvider<"http://localhost:8084", "http_headers1.headerfile">
...
module SimpleOperation =
    let operation = 
        Provider.Operation<"""query HeroQuery {
            hero (id: "1000") {
              name
              appearsIn
              homePlanet
    ...
    type Operation = Provider.Operations.HeroQuery
...
let ``Should be able to start a simple query operation synchronously with custom HTTP headers`` () =
    let headers = HttpHeaders.ofFile "http_headers2.headerfile"
    let context = Provider.GetContext(headers, "http://localhost:8084")
