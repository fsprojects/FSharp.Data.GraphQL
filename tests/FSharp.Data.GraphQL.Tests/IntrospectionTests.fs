...
type IntrospectionResult = { __schema : IntrospectionSchema }

type IntrospectionData = { Data : IntrospectionResult }
let inputFieldQuery =
    """{
  __type(name: "Query") {
    fields {
...
let ``Input field must be marked as nullable when defaultValue is provided`` () =
    ...
    let result =
        sync
        <| Executor(schema).AsyncExecute(inputFieldQuery, getMockInputContext)
    let expected =
                            ...
                            NameValueLookup.ofList [
                                ...
                                upcast
                                    [
                                        NameValueLookup.ofList [
                                            "name", upcast "inInt"
                                            "type", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "Int" ]
                                            "defaultValue", upcast "1"
                                        ]
                                        NameValueLookup.ofList [
                                            "name", upcast "inString"
                                            "type", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String" ]
...
let ``Input field must be marked as non-nullable when defaultValue is not provided`` () =
    let root =
        Define.Object (
            "Query",
            [
                Define.Field ("onlyField", StringType, "The only field", [ Define.Input ("in", StringType) ], fun _ _ -> "Only value")
            ]
        )
    let schema = Schema (root)
    let result =
                            ...
                            NameValueLookup.ofList [
                                ...
                                upcast
                                    [
                                        NameValueLookup.ofList [
                                            "name", upcast "in"
                                            "type", upcast NameValueLookup.ofList [ "kind", upcast "NON_NULL"; "name", null ]
                                            "defaultValue", null
                                        ]
                                    ]
                            ]
...
let ``Input field must be marked as nullable when its type is nullable`` () =
    let root =
        Define.Object (
            "Query",
            [
                Define.Field ("onlyField", StringType, "The only field", [ Define.Input ("in", Nullable StringType) ], fun _ _ -> "Only value")
            ]
        )
    let schema = Schema (root)
    let result =
...
let ``Introspection schema must be serializable back and forth using json`` () =
    ...
    let result =
        Executor(schema).AsyncExecute(query, getMockInputContext)
        |> sync
    ensureDirect result
    <| fun data errors ->
        empty errors
        ...
        let skippableOptions =
            ...
            options
            |> Json.configureSerializerOptions skippableOptions additionalConverters
            options
...
let ``Core type definitions are considered nullable`` () =
    ...
    let expected =
        NameValueLookup.ofList [
            "__type", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String"; "ofType", null ]
        ]
    ensureDirect result
    <| fun data errors ->
        empty errors
...
let ``Introspection works with query and mutation sharing same generic param`` () =
    let user =
        Define.Object<User>("User", [ Define.AutoField ("firstName", StringType); Define.AutoField ("lastName", StringType) ])
    let userInput = Define.InputObject<UserInput>("UserInput", [ Define.Input ("name", StringType) ])
    let query =
        Define.Object<User list>(
            "Query",
            [
    ...
    Executor(schema).AsyncExecute(IntrospectionQuery.Definition, getMockInputContext)
    |> sync
    |> ignore
                            ...
                            <| NameValueLookup.ofList [
                                ...
                                upcast
                                    NameValueLookup.ofList [
                                        ...
                                        "name", null
                                        "ofType", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String"; "ofType", null ]
                                    ]
                            ]
                        ]
                ]
                ...
                NameValueLookup.ofList [
                    ...
                    upcast
                        [
                            ...
                            <| NameValueLookup.ofList [
                                "name", upcast "onlyField"
                                "type", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String"; "ofType", null ]
                            ]
                        ]
                ]
        ]
...
let ``Default field args type definitions are considered non-null`` () =
    let root =
        Define.Object (
            "Query",
            [
                Define.Field ("onlyField", StringType, "", [ Define.Input ("onlyArg", IntType) ], fun _ () -> null)
            ]
        )
    let schema = Schema (root)
    let query =
                                        ...
                                        <| NameValueLookup.ofList [
                                            ...
                                            upcast
                                                NameValueLookup.ofList [
                                                    ...
                                                    "name", null
                                                    "ofType",
                                                    upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "Int"; "ofType", null ]
                                                ]
                                        ]
                                    ]
                            ]
...
let ``Nullable field args type definitions are considered nullable`` () =
    let root =
        Define.Object (
            "Query",
            [
                Define.Field ("onlyField", StringType, "", [ Define.Input ("onlyArg", Nullable IntType) ], fun _ () -> null)
            ]
        )
    let schema = Schema (root)
    let query =
                            ...
                            <| NameValueLookup.ofList [
                                ...
                                upcast
                                    [
                                        ...
                                        <| NameValueLookup.ofList [
                                            "name", upcast "onlyArg"
                                            "type", upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "Int"; "ofType", null ]
                                        ]
                                    ]
                            ]
                        ]
...
let ``StructNullable field args type definitions are considered nullable`` () =
    let root =
        Define.Object (
            "Query",
            [
                Define.Field ("onlyField", StringType, "", [ Define.Input ("onlyArg", StructNullable IntType) ], fun _ () -> null)
            ]
        )
    let schema = Schema (root)
    let query =
...
let ``Introspection executes an introspection query`` () =
    ...
    let result =
        sync
        <| Executor(schema).AsyncExecute(parse IntrospectionQuery.Definition, getMockInputContext, raw)
    let expected =
        NameValueLookup.ofList [
            "__schema",
            upcast
                NameValueLookup.ofList [
                    "queryType", upcast NameValueLookup.ofList [ "name", upcast "QueryRoot" ]
                    "mutationType", null
                                    ...
                                    upcast
                                        [
                                            ...
                                            <| NameValueLookup.ofList [
                                                ...
                                                "description",
                                                upcast "If this server supports mutation, the type that mutation operations will be rooted at."
                                                "args", upcast []
                                                "type",
                                                upcast NameValueLookup.ofList [ "kind", upcast "OBJECT"; "name", upcast "__Type"; "ofType", null ]
                                                "isDeprecated", upcast false
                                                "deprecationReason", null
                                            ]
                                            box
                                            ...
                                            <| NameValueLookup.ofList [
                                                ...
                                                "description",
                                                upcast "If this server support subscription, the type that subscription operations will be rooted at."
                                                "args", upcast []
                                                "type",
                                                upcast NameValueLookup.ofList [ "kind", upcast "OBJECT"; "name", upcast "__Type"; "ofType", null ]
                                                "isDeprecated", upcast false
                                        ...
                                        [
                                            ...
                                            upcast
                                                NameValueLookup.ofList [
                                                    ...
                                                    "args", upcast []
                                                    "type",
                                                    upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String"; "ofType", null ]
                                                    "isDeprecated", upcast false
                                                    "deprecationReason", null
                                                ]
                                            upcast
                                        ...
                                        [
                                            ...
                                            upcast
                                                NameValueLookup.ofList [
                                                    "name", upcast "OBJECT"
                                                    "description",
                                                    upcast "Indicates this type is an object. `fields` and `interfaces` are valid fields."
                                                    "isDeprecated", upcast false
                                                    "deprecationReason", null
                                                ]
                                            upcast
                                                NameValueLookup.ofList [
                                                    ...
                                                    "description",
                                                    upcast "Indicates this type is an interface. `fields` and `possibleTypes` are valid fields."
                                                    "isDeprecated", upcast false
                                                    "deprecationReason", null
                                                ]
                                            upcast
                    ...
                    upcast
                        [
                            ...
                            <| NameValueLookup.ofList [
                                ...
                                "locations", upcast [ box <| "FIELD"; upcast "FRAGMENT_SPREAD"; upcast "INLINE_FRAGMENT" ]
                                "args",
                                upcast
                                    [
                                        box
                                        <| NameValueLookup.ofList [
                                            ...
                                            upcast
                                                NameValueLookup.ofList [
                                                    ...
                                                    "ofType",
                                                    upcast
                                                        NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "Boolean"; "ofType", null ]
                                                ]
                                            "defaultValue", null
                                        ]
                                    ]
                            ...
                            upcast
                                NameValueLookup.ofList [
                                    ...
                                    "locations",
                                    upcast [ box <| "FIELD"; upcast "FRAGMENT_DEFINITION"; upcast "FRAGMENT_SPREAD"; upcast "INLINE_FRAGMENT" ]
                                    "args",
                                    upcast
                                        [
                                            box
                                            <| NameValueLookup.ofList [
                                                "name", upcast "label"
                                                "description", upcast "An optional label identifying the deferred payload."
                                                "type",
                                                upcast NameValueLookup.ofList [ "kind", upcast "SCALAR"; "name", upcast "String"; "ofType", null ]
                                                "defaultValue", null
                                            ]
