...
let ``Input field must be marked as nullable when defaultValue is provided`` () =
    ...
    let result = sync <| Executor(schema).AsyncExecute(inputFieldQuery, mockInputContext)
...
let ``Introspection schema must be serializable back and forth using json`` () =
    ...
    let result = Executor(schema).AsyncExecute(query, mockInputContext) |> sync
    ensureDirect result <| fun data errors ->
        empty errors
...
let ``Introspection works with query and mutation sharing same generic param`` () =
    ...
    Executor(schema).AsyncExecute(IntrospectionQuery.Definition, mockInputContext) |> sync |> ignore
...
let ``Introspection executes an introspection query`` () =
    ...
    let result = sync <| Executor(schema).AsyncExecute(parse IntrospectionQuery.Definition, mockInputContext, raw)
    let expected =
      NameValueLookup.ofList [
