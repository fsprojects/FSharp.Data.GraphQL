...
let ``Execute handles variables with complex inputs`` () =
    ...
    let variables = """{"a":"foo","b":["bar"],"c":"baz","d":null,"e":null}"""
    let params' = JsonDocument.Parse(variables).RootElement.Deserialize<ImmutableDictionary<string, JsonElement>>()
    let actual = sync <| Executor(schema).AsyncExecute(ast, variables = params')
    let expected = NameValueLookup.ofList [ "fieldWithObjectInput", upcast variables ]
    match actual with
    | Direct(data, errors) ->
      empty errors
      data |> equals (upcast expected)
