...
let ``Deserializes client subscription correctly`` () =
    ...
    | Subscribe (id, payload) ->
        ...
        | Field watchMoonField ->
            ...
            | StringValue theValue -> Assert.Equal ("1", theValue)
            | other -> Assert.Fail ($"expected arg to be a StringValue, but it was: %A{other}")
            Assert.Equal (3, watchMoonField.SelectionSet.Length)
            match watchMoonField.SelectionSet.[0] with
            | Field firstField -> Assert.Equal ("id", firstField.Name)
            | other -> Assert.Fail ($"expected field to be a Field, but it was: %A{other}")
            match watchMoonField.SelectionSet.[1] with
            | Field secondField -> Assert.Equal ("name", secondField.Name)
            | other -> Assert.Fail ($"expected field to be a Field, but it was: %A{other}")
            match watchMoonField.SelectionSet.[2] with
            | Field thirdField -> Assert.Equal ("isMoon", thirdField.Name)
            | other -> Assert.Fail ($"expected field to be a Field, but it was: %A{other}")
        | somethingElse -> Assert.Fail ($"expected it to be a field, but it was: %A{somethingElse}")
    | other -> Assert.Fail ($"unexpected actual value: '%A{other}'")
