...
let TestInterface =
    Define.Interface<ITestInterface> (
        "TestInterface",
        [
            Define.Field ("property", StringType)
            Define.Field ("method", StringType, "Test method", [ Define.Input ("x", IntType); Define.Input ("y", StringType) ], (fun _ _ -> ""))
        ]
    )
...
let ``Validation must inform about fields with not matching signatures`` () =
    let TestData =
        Define.Object<TestDataType> (
            name = "TestData",
            fields = [
                Define.Field ("property", IntType, (fun _ d -> 1))
...
let ``Validation must succeed if object implements interface correctly`` () =
    let TestData =
        Define.Object<TestDataType> (
            name = "TestData",
            fields = [
                Define.Field ("property", StringType, (fun _ d -> d.TestProperty))
                Define.Field (
                    "method",
                    StringType,
                    "Test method",
                    [ Define.Input ("x", IntType); Define.Input ("y", StringType) ],
                    (fun _ _ -> "res")
                )
            ],
            interfaces = [ TestInterface ]
