...
let execute (query : Document) = executor.AsyncExecute (query, mockInputContext) |> sync

let executeWithVariables (query : Document, variables : ImmutableDictionary<string, JsonElement>) =
    executor.AsyncExecute (ast = query, inputContext = mockInputContext, variables = variables)
    |> sync
...
let executeAndVerifyFilter (query : Document, variables : ImmutableDictionary<string, JsonElement>, filterToVerify : ObjectListFilter) =
    ...
    ex.AsyncExecute (ast = query, inputContext = mockInputContext, variables = variables) |> sync
