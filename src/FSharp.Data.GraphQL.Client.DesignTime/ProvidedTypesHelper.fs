...
module internal ProvidedScalar =
    let makeProvidedType(name) =
        let tdef = ProvidedTypeDefinition(name, Some typeof<String>, nonNullable = false, isErased = true, isSealed = true)
        tdef
    ...
    let getSchemaProvidedTypes(schema : IntrospectionSchema, uploadInputTypeName : string option) =
        ...
        and resolveProvidedType (itype : IntrospectionType) : ProvidedTypeDefinition =
            ...
            then (!providedTypes).[itype.Name]
                ...
                | TypeKind.SCALAR ->
                    let tdef = ProvidedScalar.makeProvidedType(itype.Name)
        ...
        let ignoredKinds = [TypeKind.LIST; TypeKind.NON_NULL]
