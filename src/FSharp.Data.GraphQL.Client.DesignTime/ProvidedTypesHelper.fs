...
module internal Provider =
    let getOperationMetadata
        ...
        let rec getProvidedType
            (providedTypes: Dictionary<Path * TypeName, ProvidedTypeDefinition>)
            (schemaTypes: Map<TypeName, IntrospectionType>)
            (path: Path)
                            ...
                            let getContextMethodDef =
                                let methodParameters =
                                    let serverUrl =
                                        match introspectionLocation with
                                        | Uri serverUrl ->
                                            ProvidedParameter("serverUrl", typeof<string>, optionalValue = serverUrl)
                                        | _ -> ProvidedParameter("serverUrl", typeof<string>)

                                    let httpHeaders =
                                        ProvidedParameter("httpHeaders", typeof<seq<string * string>>, optionalValue = null)

                                    let connectionFactory =
                                        ProvidedParameter(
                                            "connectionFactory",
                                            typeof<unit -> GraphQLClientConnection>,
                                            optionalValue = null
                                        )
                                ...
                                ProvidedMethod(
                                    "GetContext",
                                    methodParameters,
                                    typeof<GraphQLProviderRuntimeContext>,
                                    invoker,
                                    isStatic = true
                                )
                            ...
                            let operationMethodDef =
                                ...
                                let instanceBuilder (methodName: string) (args: obj []) =
                                    ...
                                    let enumProvidedTypes =
                                        schemaProvidedTypes |> Map.filter (fun _ t -> t.BaseType = typeof<EnumBase>)
