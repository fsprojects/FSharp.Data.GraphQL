    ...
    let makeProvidedType (tdef : ProvidedRecordTypeDefinition, properties : RecordPropertyMetadata list, explicitOptionalParameters : bool) =
            ...
            |> List.map (fun metadata ->
                ...
                let getterCode (args : Expr list) =
                    <@@
                        ...
                        match
                            this.GetProperties ()
                            |> List.tryFind (fun prop -> prop.Name = pname)
                        with
                ...
                |> Option.iter pdef.AddObsoleteAttribute
                pdef))
            ...
            tdef.AddMembersDelayed (fun _ ->
                ...
                if explicitOptionalParameters then
                    ...
                    |> List.map (fun (optionalProperties, nullValuedProperties) ->
                        let constructorProperties = requiredProperties @ optionalProperties
                        let propertyNames =
                            (constructorProperties @ nullValuedProperties)
                            |> List.map (fst >> (fun x -> x.FirstCharUpper ()))
                        let constructorPropertyTypes = constructorProperties |> List.map snd
        ...
        | :? ProvidedRecordTypeDefinition as bdef ->
            ...
            let propertiesGetter () =
                bdef.GetRecordProperties () @ properties
                |> List.map (fun p -> p.Name, p.Alias, p.Type)
...
module internal ProvidedOperationResult =
    let makeProvidedType (operationType : Type) =
        let tdef =
            ProvidedTypeDefinition ("OperationResult", Some typeof<OperationResultBase>, nonNullable = true)
        ...
        tdef.AddMembersDelayed (fun _ ->
            ...
            let variables =
                let rec mapVariable (variableName : string) (variableType : InputType) =
                    ...
                    | NamedType typeName ->
                        ...
                        | Some uploadInputTypeName when typeName = uploadInputTypeName -> variableName, TypeMapping.makeOption typeof<Upload>
                        | _ ->
            ...
            let methodOverloadDefinitions =
                ...
                let overloadsWithContext =
                    ...
                    |> List.map (fun var ->
                        ("runtimeContext", typeof<GraphQLProviderRuntimeContext>)
                        :: var)
                match contextInfo with
                ...
                let rec existsUploadType (foundTypes : ProvidedTypeDefinition list) (t : Type) =
                    ...
                    | :? ProvidedTypeDefinition as tdef when not (List.contains tdef foundTypes) ->
                        ...
                        |> Seq.exists (
                            ...
                            >> existsUploadType (tdef :: foundTypes)
                        )
            ...
            let runMethodOverloads : MemberInfo list =
                ...
                |> List.map (fun overloadParameters ->
                    let variableNames =
                        overloadParameters
                        |> List.map fst
                        |> List.filter (fun name -> name <> "runtimeContext")
                    let invoker (args : Expr list) =
                        ...
                        <@@
                            ...
                            let responseJson =
                                Tracer.runAndMeasureExecutionTime "Parsed a GraphQL response to a JsonValue" (fun _ ->
                                    JsonValue.Parse responseString)
                    ...
                    let invoker (args : Expr list) =
                        ...
                        <@@
                            ...
                            async {
                                ...
                                let! response =
                                    if shouldUseMultipartRequest then
                                        Tracer.asyncRunAndMeasureExecutionTime "Ran a multipart GraphQL query request asynchronously" (fun _ ->
                                            GraphQLClient.sendMultipartRequestAsync ct context.Connection request
                                            |> Async.AwaitTask)
                                    else
                                        Tracer.asyncRunAndMeasureExecutionTime "Ran a GraphQL query request asynchronously" (fun _ ->
                                            GraphQLClient.sendRequestAsync ct context.Connection request
                                            |> Async.AwaitTask)
            ...
            let members : MemberInfo list =
                [ operationResultDef; parseResultDef ]
                @ runMethodOverloads
...
module internal Provider =
    ...
    let getOperationMetadata
        (
            schemaTypes : Map<TypeName, IntrospectionType>,
            uploadInputTypeName : string option,
            enumProvidedTypes : Map<TypeName, ProvidedTypeDefinition>,
            operationAstFields,
            operationTypeRef,
            explicitOptionalParameters : bool
        ) =
        let generateWrapper name =
            let rec resolveWrapperName actual =
                if schemaTypes.ContainsKey (actual) then
                    resolveWrapperName (actual + "Fields")
                else
                    actual
        ...
        let rec getProvidedType
            (providedTypes : Dictionary<FieldStringPath * TypeName, ProvidedTypeDefinition>)
            (schemaTypes : Map<TypeName, IntrospectionType>)
            (path : FieldStringPath)
            (astFields : AstFieldInfo list)
            (tref : IntrospectionTypeRef)
            : Type =
            match tref.Kind with
            | TypeKind.SCALAR when tref.Name.IsSome ->
                TypeMapping.mapScalarType uploadInputTypeName tref.Name.Value
                |> TypeMapping.makeOption
            | _ when
                uploadInputTypeName.IsSome
                && tref.Name.IsSome
                && uploadInputTypeName.Value = tref.Name.Value
                ->
            ...
            | TypeKind.ENUM when tref.Name.IsSome ->
                ...
                | None ->
                    failwith
                        $"""Could not find a enum type based on a type reference. The reference is an "%s{tref.Name.Value}" enum, but that enum was not found in the introspection schema."""
                ...
                if providedTypes.ContainsKey (path, tref.Name.Value) then
                    ...
                    let getIntrospectionFields typeName =
                        if schemaTypes.ContainsKey (typeName) then
                            ...
                            failwith
                                $"""Could not find a schema type based on a type reference. The reference is to a "%s{typeName}" type, but that type was not found in the schema types."""
                    let getPropertyMetadata typeName (info : AstFieldInfo) : RecordPropertyMetadata =
                        let ifield =
                            match
                                getIntrospectionFields typeName
                                |> Array.tryFind (fun f -> f.Name = info.Name)
                            with
                            | Some ifield -> ifield
                            | None ->
                                failwith
                                    $"""Could not find field "%s{info.Name}" of type "%s{tref.Name.Value}". The schema type does not have a field with the specified name."""
                    ...
                    let createFragmentType (typeName, properties) =
                        ...
                        includeType path tdef
                        ProvidedRecord.makeProvidedType (tdef, properties, explicitOptionalParameters)
                        |> ignore
    ...
    let getSchemaProvidedTypes (schema : IntrospectionSchema, uploadInputTypeName : string option, explicitOptionalParameters : bool) =
        ...
        and resolveInputFieldMetadata (field : IntrospectionInputVal) : RecordPropertyMetadata =
            ...
            | TypeKind.NON_NULL when field.Type.Name.IsNone && field.Type.OfType.IsSome ->
                ofInputFieldType field
                |> resolveInputFieldMetadata
                |> unwrapOption
        ...
        schemaTypes
        |> Map.iter (fun _ itype ->
            if not (List.contains itype.Kind ignoredKinds) then
                resolveProvidedType itype |> ignore)
        ...
        |> Seq.iter (fun kvp ->
            if
                kvp.Value.Kind = TypeKind.INTERFACE
                || kvp.Value.Kind = TypeKind.UNION
            then
                let itype = getProvidedType kvp.Value.Name
                let ptypes = possibleTypes kvp.Value
                ptypes
                |> Array.iter (fun ptype -> ptype.AddInterfaceImplementation (itype)))
        providedTypes.Value
                            ...
                            let getContextMethodDef =
                                let methodParameters =
                                    ...
                                    let connectionFactory =
                                        ProvidedParameter ("connectionFactory", typeof<unit -> GraphQLClientConnection>, optionalValue = null)
                            ...
                            let operationMethodDef =
                                ...
                                let instanceBuilder (methodName : string) (args : obj[]) =
                                    ...
                                    let operationTypeRef =
                                        let tref =
                                            ...
                                            | Mutation ->
                                                ...
                                                | Some tref -> tref
                                                | None ->
                                                    failwith "The operation is a mutation operation, but the schema does not have a mutation type."
                                            | Subscription ->
                                    ...
                                    let enumProvidedTypes =
                                        schemaProvidedTypes
                                        |> Map.filter (fun _ t -> t.BaseType = typeof<EnumBase>)
                                    let actualQuery =
                                        queryAst
