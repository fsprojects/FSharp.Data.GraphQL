...
module GraphQLQueryDecoding =
    ...
    let genericErrorContentForDoc (docId : int) (message : string) = struct (docId, [ GQLProblemDetails.Create (message, Skip) ])

    let genericFinalErrorForDoc (docId : int) (message : string) = Result.Error (genericErrorContentForDoc docId message)

    let genericFinalError message = message |> genericFinalErrorForDoc -1

    let private resolveVariables
        (serializerOptions : JsonSerializerOptions)
        (expectedVariables : Types.VarDef list)
        (variableValuesObj : JsonDocument)
        =
        result {
            try
                try
                    if (not (variableValuesObj.RootElement.ValueKind.Equals (JsonValueKind.Object))) then
                        ...
                        let providedVariableValues =
                            variableValuesObj.RootElement.EnumerateObject ()
                            |> List.ofSeq
                        return!
                            Ok (
                                expectedVariables
                ...
                with
                | :? JsonException as ex -> return! Result.Error (ex.Message)
                | :? GraphQLException as ex -> return! Result.Error (ex.Message)
                | ex ->
    ...
    let decodeGraphQLQuery
        ...
        (query : string)
        =
        let executionPlanResult = result {
            try
                match operationName with
                | Some operationName -> return! executor.CreateExecutionPlan (query, operationName = operationName)
                | None -> return! executor.CreateExecutionPlan (query)
            with
            | :? JsonException as ex -> return! genericFinalError (ex.Message)
            | :? GraphQLException as ex -> return! genericFinalError (ex.Message)
        }

        executionPlanResult
        |> Result.bind (fun executionPlan ->
        ...
        |> Result.map (fun (executionPlan, variables) -> { ExecutionPlan = executionPlan; Variables = variables })
