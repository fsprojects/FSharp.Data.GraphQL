...
module HttpHandlers =
    ...
    let private httpOk
        ...
        >=> (fun _ ctx ->
            JsonSerializer
                .SerializeAsync(ctx.Response.Body, payload, options = serializerOptions, cancellationToken = cancellationToken)
    ...
    let private prepareGenericErrors (errorMessages : string list) =
        (NameValueLookup.ofList [
            ...
            upcast
                (errorMessages
                 |> List.map (fun msg -> NameValueLookup.ofList [ "message", upcast msg ]))
        ])
        ...
        task {
            let cancellationToken =
                CancellationTokenSource
                    .CreateLinkedTokenSource(cancellationToken, ctx.RequestAborted)
                    .Token
            if cancellationToken.IsCancellationRequested then
                ...
                let deserializeGraphQLRequest () = task {
                    try
                        let! deserialized = JsonSerializer.DeserializeAsync<GraphQLRequest> (ctx.Request.Body, serializerOptions)
                        return Ok deserialized
                    with :? GraphQLException as ex ->
                        logger.LogError (``exception`` = ex, message = "Error while deserializing request.")
                        return Result.Error [ $"%s{ex.Message}\n%s{ex.ToString ()}" ]
                }

                let applyPlanExecutionResult (result : GQLExecutionResult) = task {
                    let gqlResponse =
                        match result.Content with
                        | Direct (data, errs) -> GQLResponse.Direct (result.DocumentId, data, errs)
                        | RequestError (problemDetailsList) -> GQLResponse.RequestError (result.DocumentId, problemDetailsList)
                        | _ ->
                            GQLResponse.RequestError (
                                result.DocumentId,
                                [
                                    GQLProblemDetails.Create ("subscriptions are not supported here (use the websocket endpoint instead).")
                                ]
                            )
                    return! httpOk cancellationToken interceptor serializerOptions gqlResponse next ctx
                }

                let handleDeserializedGraphQLRequest (graphqlRequest : GraphQLRequest) = task {
                    match graphqlRequest.Query with
                    | None ->
                        let! result =
                            executor.AsyncExecute (IntrospectionQuery.Definition)
                            |> Async.StartAsTask
                    ...
                    | Some queryAsStr ->
                        ...
                        | Result.Error struct (docId, probDetails) ->
                            return! httpOk cancellationToken interceptor serializerOptions (GQLResponse.RequestError (docId, probDetails)) next ctx
                        | Ok query ->
                            if logger.IsEnabled (LogLevel.Debug) then
                            ...
                            let! result =
                                ...
                                executor.AsyncExecute (query.ExecutionPlan, data = root, variables = variables)
                                |> Async.StartAsTask
                            if logger.IsEnabled (LogLevel.Debug) then
                                logger.LogDebug ($"Result metadata: %A{result.Metadata}")
                            else
                ...
                if ctx.Request.Headers.ContentLength.GetValueOrDefault (0) = 0 then
                    ...
                    | Ok graphqlRequest -> return! handleDeserializedGraphQLRequest graphqlRequest
        }

    /// HttpHandler for handling GraphQL requests with Giraffe
    let handleGraphQL<'Root> (cancellationToken : CancellationToken) (logger : ILogger) (next : HttpFunc) (ctx : HttpContext) =
        handleGraphQLWithResponseInterception<'Root> cancellationToken logger id next ctx
