        ...
        |> List.fold(fun (fields : ExecutionInfo list, deferredFields : DeferredExecutionInfo list) selection ->
            ...
            | Field field ->
                ...
                then fields, deferredFields
                    ...
                    match field with
                    | Deferred -> (fields, { Info = { info with Kind = SelectFields [executionPlan] }; Path = path'; Kind = DeferredExecution; DeferredFields = deferredFields'} :: deferredFields)
                    | Streamed -> (fields, { Info = { info with Kind = SelectFields [executionPlan] }; Path = path'; Kind = StreamedExecution; DeferredFields = deferredFields'} :: deferredFields)
    ...
    let plannedTypeFields, deferredFields' =
        ...
        |> List.fold(fun (fields : Map<string, ExecutionInfo list>, deferredFields : DeferredExecutionInfo list) selection ->
            ...
            | Field field ->
                ...
                match field with
                | Deferred -> fields, { Info = { innerData with Kind = ResolveAbstraction infoMap }; Path = path'; Kind = DeferredExecution; DeferredFields = deferredFields' } :: deferredFields
                | Streamed -> fields, { Info = { innerData with Kind = ResolveAbstraction infoMap }; Path = path'; Kind = StreamedExecution; DeferredFields = deferredFields' } :: deferredFields
