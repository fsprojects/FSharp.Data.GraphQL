...
let private resolveField (execute : ExecuteField) (ctx : ResolveFieldContext) (parentValue : obj) =
    if ctx.ExecutionInfo.IsNullable then
        execute ctx parentValue |> AsyncVal.map (optionCast)
    else
...
let private nullResolverError name path ctx =
    resolverError
        ...
        ctx
        (GQLMessageException
         <| sprintf "Non-Null field %s resolved as a null!" name)
let private coercionError value tyName path ctx =
    resolverError
        path
        ctx
        (GQLMessageException
         <| sprintf "Value '%O' could not be coerced to scalar %s" value tyName)
let private interfaceImplError ifaceName tyName path ctx =
    resolverError
        path
        ctx
        (GQLMessageException
         <| sprintf "GraphQL Interface '%s' is not implemented by the type '%s'" ifaceName tyName)
let private unionImplError unionName tyName path ctx =
    resolverError path ctx (GQLMessageException (sprintf "GraphQL Union '%s' is not implemented by the type '%s'" unionName tyName))
let private deferredNullableError name tyName path ctx =
    resolverError path ctx (GQLMessageException (sprintf "Deferred field %s of type '%s' must be nullable" name tyName))
let private streamListError name tyName path ctx =
    resolverError path ctx (GQLMessageException (sprintf "Streamed field %s of type '%s' must be list" name tyName))
...
let private ownDeferredResult
    ...
    : IObservable<GQLDeferredResponseContent> * IObservable<GQLDeferredResponseContent> option =
...
let rec private direct
    ...
    | Enum enumDef ->
        let enumCase =
            enumDef.Options
            |> Array.tryPick (fun case -> if case.Value.Equals (value) then Some case.Name else None)
        match enumCase with
    ...
    | Nullable (Output innerDef) ->
        ...
        |> AsyncVal.map (
            Result.valueOr (fun errs -> (KeyValuePair (name, null), None, errs))
            >> Ok
        )
...
and private streamed
    ...
    let resolveItem index item = asyncVal {
        let! result =
            executeResolvers inputContext innerCtx (box index :: path) parent (toOption item |> AsyncVal.wrap)
        return (index, result)
    }
