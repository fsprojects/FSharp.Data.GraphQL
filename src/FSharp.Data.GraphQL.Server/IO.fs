...
and GQLResponseContent =
    /// The request was rejected before execution started: validation, planning, variable coercion, a middleware,
    /// or the executor itself failing. There is no data, unlike a Direct result whose data happens to be null.
    | RequestError of Errors: GQLProblemDetails list
    /// An execution result. Data is null when a non-null root field failed and the error propagated to the root,
    /// exactly as it would for a non-null nested field, rather than being rejected as a RequestError.
