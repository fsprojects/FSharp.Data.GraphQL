        ...
        | Union uniondef ->
            let nonEmptyResult =
                if uniondef.Options.Length > 0 then
                    ...
                    ValidationError [ uniondef.Name + " must have at least one type definition option" ]
...
module Ast =
    ...
    type SchemaInfo =
        ...
        member x.TryGetInputType(input: TypeReference) =
            match input with
            | NamedType name ->
            ...
            | NonNullNameType inner -> x.TryGetInputType(inner) |> Option.map IntrospectionTypeRef.NonNull
    ...
    type SelectionInfoContext =
        ...
        member x.FragmentOrParentType =
            x.FragmentType |> Option.map (fun x -> x.TypeCondition) |> Option.defaultValue x.ParentType
    ...
    let private onAllSelections (ctx: ValidationContext) (onSelection: SelectionInfo -> ValidationResult<AstError>) =
        let rec traverseSelections selection =
            (onSelection selection) @@ (selection.SelectionSet |> collectResults traverseSelections)
        ...
        |> List.collect (function
            | Field field ->
                let introspectionField =
                    ctx.FragmentOrParentType.Fields |> tryFindInArrayOption (fun f -> f.Name = field.Name)
        ...
        if fieldA.FieldType = fieldB.FieldType then
            ...
            |> collectResults (fun (KeyValue (_, selectionSet)) ->
                if selectionSet.Length < 2 then
                    ...
                    List.pairwise selectionSet |> collectResults sameResponseShape)
        else
            AstError.AsResult(
            ...
            if selectionSet.Length < 2 then
                ...
                |> collectResults (fun (fieldA, fieldB) ->
                    ...
                    if fieldA.FragmentOrParentType = fieldB.FragmentOrParentType
                       ...
                       || fieldB.FragmentOrParentType.Kind <> TypeKind.OBJECT then
    ...
    let rec private validateArgumentUniquenessInSelection (selection: SelectionInfo) =
        ...
        let directiveArgsValid =
            selection.Field.Directives
            |> collectResults (fun directive -> validateArgs $"directive '%s{directive.Name}'" selection.Path directive.Arguments)
    ...
    and private checkFragmentTypeExistenceInSelection
        (fragmentDefinitions: FragmentDefinition list)
        (schemaInfo: SchemaInfo)
        (path: Path)
        =
    ...
    let internal validateFragmentsMustBeUsed (ctx: ValidationContext) =
        ...
        let fragmentSpreadNames =
            ...
            |> Set.collect (fun def -> Set.ofList def.SelectionSet |> Set.collect (getSpreadNames Set.empty))

        getFragmentDefinitions ctx.Document
        |> collectResults (fun def ->
            if def.Name.IsSome && Set.contains def.Name.Value fragmentSpreadNames then
                Success
            else
                AstError.AsResult
    ...
    let rec private fragmentSpreadTargetDefinedInSelection (fragmentDefinitionNames: string list) (path: Path) =
        ...
        | FragmentSpread spread ->
            if List.contains spread.Name fragmentDefinitionNames then
                ...
                AstError.AsResult(
                    $"Fragment spread '%s{spread.Name}' refers to a non-existent fragment definition in the document.",
                    path
                )
    ...
    let private checkFragmentSpreadIsPossibleInSelection
        (
            path: Path,
            parentType: IntrospectionType,
            fragmentType: IntrospectionType
        ) =
    ...
    let rec private getFragmentAndParentTypes (set: SelectionInfo list) =
        ...
        ||> List.fold (fun acc selection ->
            match selection.FragmentType with
            | Some fragType when fragType.Name <> selection.ParentType.Name ->
                (selection.Path, selection.ParentType, fragType) :: acc
            | _ -> acc)
    ...
    let private checkInputValue (schemaInfo: SchemaInfo) (variables: VariableDefinition list option) (selection: SelectionInfo) =
        let rec getFieldMap (fields: (string * IntrospectionTypeRef) seq) : Map<string, IntrospectionTypeRef> =
            (Map.empty, fields) ||> Seq.fold (fun acc (name, tref) -> Map.add name tref acc)
        ...
        let rec checkIsCoercible (tref: IntrospectionTypeRef) (argName: string) (value: Value) =
            ...
            | ObjectValue props ->
                ...
                | TypeKind.INPUT_OBJECT when tref.Name.IsSome ->
                    ...
                    | Some itype ->
                        let fieldMap =
                            itype.InputFields |> Option.defaultValue [||] |> Array.map (fun x -> x.Name, x.Type) |> getFieldMap

                        let canCoerceFields =
                            fieldMap
                            |> collectResults (fun kvp ->
                                if kvp.Value.Kind = TypeKind.NON_NULL && not (props.ContainsKey(kvp.Key)) then
                                    AstError.AsResult(
            ...
            | Variable varName ->
                let variableDefinition =
                    ...
                    |> Option.defaultValue []
                    |> List.tryPick (fun v ->
                        if v.VariableName = varName then Some(v, schemaInfo.TryGetInputType(v.Type)) else None)
        ...
        |> collectResults (fun arg ->
            let argumentTypeRef =
                selection.InputValues |> Array.tryPick (fun x -> if x.Name = arg.Name then Some x.Type else None)
    ...
    let rec private getDistinctDirectiveNamesInSelection (path: Path) (selection: Selection) : (Path * Set<string>) list =
        ...
        | Field field ->
            ...
            let selectionSetDirectives =
                field.SelectionSet |> List.collect (getDistinctDirectiveNamesInSelection path)
    ...
    and private getDistinctDirectiveNamesInDefinition (path: Path) (frag: Definition) : (Path * Set<string>) list =
        ...
        let selectionSetDirectives =
            frag.SelectionSet |> List.collect (getDistinctDirectiveNamesInSelection path)
    ...
    let internal validateDirectivesDefined (ctx: ValidationContext) =
        ...
        |> collectResults (fun (path, names) ->
            ...
            |> collectResults (fun name ->
                if ctx.Schema.Directives |> Array.exists (fun x -> x.Name = name) then
                    Success
                else
    ...
    let internal validateUniqueDirectivesPerLocation (ctx: ValidationContext) =
        ...
        |> List.collect (fun def ->
            ...
            let selectionSetDirectives =
                def.Definition.SelectionSet |> List.collect (getDirectiveNamesInSelection path)
        ...
        |> collectResults (function
            | OperationDefinition def ->
                ...
                |> collectResults (fun (var, count) ->
                    ...
                    | Some operationName ->
                        AstError.AsResult
                            $"Variable '%s{var.VariableName}' in operation '%s{operationName}' is declared %i{count} times. Variables must be unique in their operations."
                    | None ->
                        AstError.AsResult
                            $"Variable '%s{var.VariableName}' is declared %i{count} times in the operation. Variables must be unique in their operations.")
            | _ -> Success)
            ...
            | OperationDefinition def ->
                ...
                |> collectResults (fun var ->
                    ...
                    | Some operationName, None ->
                        AstError.AsResult
                            $"Variable '%s{var.VariableName}' in operation '%s{operationName}' has a type that is not an input type defined by the schema (%s{var.Type.ToString()})."
                    | None, None ->
                        AstError.AsResult
                            $"Variable '%s{var.VariableName}' has a type is not an input type defined by the schema (%s{var.Type.ToString()})."
                    | _ -> Success)
            | _ -> Success)
        ...
        |> collectResults (fun arg ->
            ...
            | Variable varName ->
                if variableDefinitions |> Set.contains varName then
                    ...
                    AstError.AsResult(
                        $"A variable '%s{varName}' is referenced in an argument '%s{arg.Name}' of directive '%s{directive.Name}' of field with alias or name '%s{path.Head}', but that variable is not defined in the operation.",
                        path
                    )
            | _ -> Success)
    ...
    let rec private checkVariablesDefinedInSelection
        ...
        | InlineFragment frag ->
            ...
            variablesValid
            @@ (frag.Directives |> collectResults (checkVariablesDefinedInDirective variableDefinitions path))
    ...
    let internal validateVariablesUsesDefined (ctx: ValidationContext) =
        ...
        |> collectResults (function
            | OperationDefinition def ->
                ...
                def.SelectionSet
                |> collectResults (checkVariablesDefinedInSelection fragmentDefinitions varNames path)
            | _ -> Success)
        ...
        if List.contains spread.Name visitedFragments then
            ...
            let usedInSpread =
                ...
                | Some frag ->
                    ...
                    usedInSelection
                    || (frag.Directives |> List.exists (fun directive -> argumentsContains name directive.Arguments))
                | None -> false

            usedInSpread
            || (spread.Directives |> List.exists (fun directive -> argumentsContains name directive.Arguments))

    and private variableIsUsedInSelection
        (name: string)
        (fragmentDefinitions: FragmentDefinition list)
        (visitedFragments: string list)
        =
        ...
        | Field field ->
            if argumentsContains name field.Arguments then
                ...
                usedInSelection
                || (field.Directives |> List.exists (fun directive -> argumentsContains name directive.Arguments))
        | InlineFragment frag ->
            let usedInSelection =
                frag.SelectionSet
                |> List.exists (variableIsUsedInSelection name fragmentDefinitions visitedFragments)
            ...
            | OperationDefinition def ->
                ...
                |> collectResults (fun varDef ->
                    ...
                    | Some operationName, _ ->
                        AstError.AsResult
                            $"Variable definition '%s{varDef.VariableName}' is not used in operation '%s{operationName}'. Every variable must be used."
                    | None, _ ->
                        AstError.AsResult
                            $"Variable definition '%s{varDef.VariableName}' is not used in operation. Every variable must be used.")
            | _ -> Success)

    let rec private areTypesCompatible (variableTypeRef: IntrospectionTypeRef) (locationTypeRef: IntrospectionTypeRef) =
        if locationTypeRef.Kind = TypeKind.NON_NULL && locationTypeRef.OfType.IsSome then
            if variableTypeRef.Kind <> TypeKind.NON_NULL then
                ...
                false
        elif variableTypeRef.Kind = TypeKind.NON_NULL && variableTypeRef.OfType.IsSome then
            areTypesCompatible variableTypeRef.OfType.Value locationTypeRef
        elif locationTypeRef.Kind = TypeKind.LIST && locationTypeRef.OfType.IsSome then
            ...
            variableTypeRef.Name = locationTypeRef.Name && variableTypeRef.Kind = locationTypeRef.Kind
        ...
        |> collectResults (fun arg ->
            ...
            | Variable varName ->
                ...
                | Some (varDef, variableTypeRef) ->
                    let err =
                        AstError.AsResult(
                            $"Variable '%s{varName}' can not be used in its reference. The type of the variable definition is not compatible with the type of its reference.",
                            path
                        )
                    ...
                    | Some input ->
                        ...
                        if locationTypeRef.Kind = TypeKind.NON_NULL
                           && locationTypeRef.OfType.IsSome
                           && variableTypeRef.Kind <> TypeKind.NON_NULL then
                            let hasNonNullVariableDefaultValue = varDef.DefaultValue.IsSome
                            let hasLocationDefaultValue = input.DefaultValue.IsSome

                            if not hasNonNullVariableDefaultValue && not hasLocationDefaultValue then
                                err
                            else
                                let nullableLocationType = locationTypeRef.OfType.Value
                                if not (areTypesCompatible variableTypeRef nullableLocationType) then err else Success
        ...
        |> collectResults (fun def ->
            let varNamesAndTypeRefs =
                ...
                |> List.choose (fun varDef ->
                    ctx.Schema.TryGetInputType(varDef.Type) |> Option.map (fun t -> varDef.VariableName, (varDef, t)))
                |> Map.ofList
