...
and SubscriptionCallback =
    | Undefined
    | Sync of result:Type * expr:Expr
    | Async of result:Type * expr:Expr

    member x.Expr = 
        match x with
        | Sync(_,e) -> e
        | Async(_,e) -> e
        | Undefined -> failwith "Callback function not defined"
...
and [<CustomEquality; NoComparison>] SubscriptionFieldDefinition<'Val, 'Input> = 
    {
        Name : string
        Description : string option
        DeprecationReason : string option
