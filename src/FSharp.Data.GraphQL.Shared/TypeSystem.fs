...
/// <summary>
/// Untyped batching policy of a <see cref="Resolve.TaskSeq"/> field, applied to its items
/// when the field is requested with the <c>@stream</c> directive.
/// </summary>
and [<RequireQualifiedAccess>] StreamBatchingPolicy =
    ...
    | BatchFromSource of getBatchSize : Func<obj, int voption>
...
and Resolve =
    ...
    /// batching defines how items are grouped when the field is streamed
    | TaskSeq of input : Type * output : Type * expr : Expr * batching : StreamBatchingPolicy
...
/// <summary>
/// Defines how items of a field created with <see cref="Define.TaskSeqField"/> are grouped into batches
/// when the field is requested with the <c>@stream</c> directive.
/// </summary>
/// <remarks>
/// The <c>preferredBatchSize</c> argument of the <c>@stream</c> directive takes precedence over this definition.
/// </remarks>
...
type StreamBatching<'Item> =
    ...
    /// Returning <see langword="ValueNone"/> delivers items as soon as they are produced.
    /// </summary>
    | FromSource of getBatchSize : (IAsyncEnumerable<'Item> -> int voption)

    /// <summary>Converts the typed batching definition into the untyped policy stored in <see cref="Resolve.TaskSeq"/>.</summary>
    static member internal ToPolicy (batching : StreamBatching<'Item> voption) : StreamBatchingPolicy =
        ...
        | ValueSome (StreamBatching.FromSource getBatchSize) ->
            StreamBatchingPolicy.BatchFromSource (Func<obj, int voption> (fun source -> getBatchSize (source :?> IAsyncEnumerable<'Item>)))
...
type internal IAsyncEnumerableFieldValue =
    ...
    abstract Items : IAsyncEnumerable<obj>
    /// Batch size computed from the batching policy of the field for this sequence.
    abstract PreferredBatchSize : int voption

/// <summary>
/// Wraps a typed asynchronous sequence returned by a field resolver.
/// Items are boxed one by one, because an asynchronous sequence of a value type is not an asynchronous sequence of <see cref="obj"/>.
/// </summary>
type internal AsyncEnumerableFieldValue<'Item> (source : IAsyncEnumerable<'Item>, [<Struct>] ?preferredBatchSize : int) =
    ...
    interface IAsyncEnumerableFieldValue with
        ...
        member _.PreferredBatchSize = preferredBatchSize
...
module Resolve =
    ...
    let private applyBatching (policy : StreamBatchingPolicy) (source : obj) =
        match policy with
        ...
        | StreamBatchingPolicy.BatchFromSource getBatchSize ->
            ...
            match getBatchSize.Invoke source with
            | ValueSome size when size > 0 -> ValueSome size
            | _ -> ValueNone

    let private wrapAsyncEnumerable<'U> (policy : StreamBatchingPolicy) (source : IAsyncEnumerable<'U>) : obj =
        match box source with
        // A null sequence is reported by the executor the same way as any other null value
        | null -> null
        | boxedSource ->
            // A voption is passed to the struct optional parameter by name only from F# compilers shipped with SDK 10.0.4xx,
            // so both cases are spelled out to keep the code buildable with SDK 10.0.3xx
    ...
    let private boxifyTaskSeq<'T, 'U> (policy : StreamBatchingPolicy) (f : ResolveFieldContext -> 'T -> IAsyncEnumerable<'U>) : ResolveFieldContext -> obj -> obj =
        let resolve (ctx : ResolveFieldContext) (x : obj) = f ctx (x :?> 'T) |> wrapAsyncEnumerable policy
        resolve

    let private boxifyTaskSeqOption<'T, 'U> (policy : StreamBatchingPolicy) (f : ResolveFieldContext -> 'T -> IAsyncEnumerable<'U> option) : ResolveFieldContext -> obj -> obj =
        let resolve (ctx : ResolveFieldContext) (x : obj) =
            match f ctx (x :?> 'T) with
            | Some source ->
                match wrapAsyncEnumerable policy source with
                | null -> null
    ...
    let private boxifyExprTaskSeq (policy : StreamBatchingPolicy) expr : ResolveFieldContext -> obj -> obj =
        let invoke (methodInfo : MethodInfo) (input : Type) (item : Type) (resolver : obj) =
            methodInfo
                .GetGenericMethodDefinition()
                .MakeGenericMethod(input, item)
                .Invoke (null, [| box policy; resolver |])
            |> unbox
        match unwrapExpr expr with
    ...
    let (|BoxedTaskSeq|_|) =
        function
        | TaskSeq (d, c, expr, policy) -> ValueSome (d, c, boxifyExprTaskSeq policy expr)
        | _ -> ValueNone
