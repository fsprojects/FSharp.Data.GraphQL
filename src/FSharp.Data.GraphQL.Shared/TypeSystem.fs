...
/// TODO: compose with InputValueDefinition
...
and [<CustomEquality; NoComparison>] SubscriptionObjectDefinition<'Val> =
    ...
    interface ObjectDef with
        ...
        member x.Implements: InterfaceDef [] = Array.empty
...
module Resolve =
    ...
    let private genMethodResolve<'Val, 'Res> (typeInfo: TypeInfo) (methodInfo: MethodInfo) =
        ...
        let args = methodInfo.GetParameters() |> Array.map argExpr |> Array.toList

        let expr =
            Expr.Lambda(ctxVar, Expr<'Val -> 'Res>.Lambda (valueVar, Expr.Call(Expr.Var(valueVar), methodInfo, args)))
    ...
    let private genPropertyResolve<'Val, 'Res> (typeInfo: TypeInfo) property =
        ...
        let expr =
            Expr.Lambda(ctxVar, Expr<'Val -> 'Res>.Lambda (valueVar, Expr.PropertyGet(Expr.Var(valueVar), property)))
...
module SchemaDefinitions =
    ...
    let private coerceLongInput =
        ...
        | StringValue s ->
            ...
            | true, i -> Some i
            | false, _ -> None
        | BooleanValue b -> Some(if b then 1L else 0L)
        | _ -> None
    ...
    let coerceBoolInput =
        function
        | IntValue i -> Some(if i = 0L then false else true)
    ...
    let Nullable (innerDef: #TypeDef<'Val>) : NullableDef<'Val> =
        upcast
            {
                NullableDefinition.OfType = innerDef
            }
    ...
    type Define =
        ...
        static member SubscriptionObject<'Val>
            (
                name: string,
                fields: SubscriptionFieldDef<'Val> list,
                ?description: string
