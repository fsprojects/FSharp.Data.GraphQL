...
/// <summary>
/// Untyped options of a <see cref="Resolve.TaskSeq"/> field, applied when the field is requested with the
/// <c>@stream</c> directive.
/// </summary>
...
type StreamBatching<'Item> =
    ...
    /// <summary>
    /// Converts the typed batching definition and the field's <c>maxConcurrency</c> into the untyped options stored
    /// in <see cref="Resolve.TaskSeq"/>. A missing <paramref name="maxConcurrency"/> defaults to
...
type internal IAsyncEnumerableFieldValue =
    ...
    /// </summary>
    /// <remarks>
    /// Evaluated lazily: only when the field is requested with the <c>@stream</c> directive and the query does not
    /// specify its own <c>preferredBatchSize</c>, so a batching callback never runs for an ordinary or deferred query.
    /// </remarks>
