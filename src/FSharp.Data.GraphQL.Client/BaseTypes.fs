...
module ResponseTypeBuilder =
    let private fail : string -> 'T = fun msg -> failwith ("Failure deserializing query response. " + msg)
    ...
    static member internal MakeProvidedType(asm, ns, schema : IntrospectionSchema, serverUrl : string, customHeaders : seq<string * string>) =
        ...
        let mdef =
            ...
            let genfn (mname : string) (args : obj []) =
                ...
                // TODO: Check if this is the best option to generate unique class name
                let className = sprintf "GraphQLQueryResult<%s>" (Guid.NewGuid().ToString("N"))
                ...
                let outputTypes =
                    ...
                    ResponseTypeBuilder.buildTypes asm ns schema operationName queryAst responseJson
                outputTypes |> Seq.iter (fun kvp -> qdef.AddMember(kvp.Value))
