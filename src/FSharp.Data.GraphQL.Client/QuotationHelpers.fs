...
module QuotationHelpers =
        ...
        let getLaunchQueryBody com args =
            ...
            .then(resp => new Promise((resolve, reject) => {
                if (resp.errors) {
                    ...
                    resolve($2 ? resp.data[$2] : resp.data);
                }
            }))"""
            |> makeEmit args
        ...
        let getBuildQueryBody com args =
            ...
            var tuple = $1
                ? (function() {
                    ...
                    return [$1.substr(0, i), $1.substr(i)];
                  }())
                : ["", ""];
            var queryFields = tuple[0], queryFragments = tuple[1];
    ...
    let launchRequest (serverUrl: string) (opName: string) (opField: string)
                    (cont: obj->'T) (reqBody: string): Async<'T> =
        failwith "JS only"
