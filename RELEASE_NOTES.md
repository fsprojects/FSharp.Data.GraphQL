...
* Added cancellation of a streamed `Define.TaskSeqField` enumeration when the client unsubscribes, and delivery of an enumeration error as a deferred error for the field after the items already produced
* Added `StreamBatching` to group streamed items of a `Define.TaskSeqField` into batches of a fixed size or of a size computed from the sequence, such as a page size kept with a paged SDK sequence. The `preferredBatchSize` argument of `@stream` takes precedence
